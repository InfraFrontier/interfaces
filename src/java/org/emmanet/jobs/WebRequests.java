...
public class WebRequests {
    ...
    public List webReqDupCheck(String emmaId, String email, String regInt) {
        List duplicatedList = null;
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
        session.beginTransaction();
        try {

            /*   regInterestList = session.createQuery("from WebRequestsDAO where strain_id = ?" +
             " and register_interest IS NOT NULL").setString(0, emmaId).list(); */
            duplicatedList = session.createQuery("SELECT id_req,register_interest FROM WebRequestsDAO WHERE strain_id = ?"
                    + " AND sci_e_mail = ? AND register_interest = ? AND req_status NOT IN ('CANC','SHIP')").setString(0, emmaId).setString(1, email).setString(2, regInt).list();
            session.getTransaction().commit();
    ...
    public List strainsUpdates(String shortDate, String fullDate) {
        ...
        try {
            ...
            updatesList = session.createQuery("from StrainsDAO where last_change" + " like '" + shortDate + "%' AND (str_access = 'P'" + " AND str_status = 'ARCHD' OR available_to_order = 'yes') AND (gp_release < '" + fullDate + "' or gp_release IS NULL)" + " order by gp_release DESC").list();
    ...
    public List strainListArrd() {
        ...
        try {
            ...
            strainList = session.createQuery("from StrainsDAO WHERE str_status IN ('ARRD', 'ARING', 'TNA') "
                    + "AND str_access = 'P' AND (available_to_order != 'yes' "
                    + "OR available_to_order IS NULL) ORDER BY emma_id").list();
            session.getTransaction().commit();
    ...
    public List requestByID(int requestId) {
        ...
        session.beginTransaction();
        try {
//used in current roi code   requestById = session.createQuery("from WebRequestsDAO where id_req = " + requestId).list();
    ...
    public WebRequestsDAO getReqByID(String id) {
        ...
        /* added from regisyerinterest app */
        WebRequests wr = new WebRequests();
        webReq.setCvDAO(wr.isoCountries());
        /* added from regisyerinterest app */
        return webReq;
    }
...
//to satisfy JIRA-219
    ...
    public StrainsDAO getStrainByID(int id) {
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
        session.beginTransaction();
        StrainsDAO sd = null;
