 ...
 * @author mrelac
 */
public class AlleleManagementDetailController extends SimpleFormController implements Validator {
    private final GenesManager genesManager = new GenesManager();
    
    public AlleleManagementDetailController() {
...
//        ModelAndView modelAndView = new ModelAndView(getSuccessView(), "gene", gene);
        ModelAndView modelAndView = new ModelAndView(new RedirectView("alleleManagementDetail.emma?id=" + gene.getId_gene() + "&action=editGene"), "gene", gene);
        String action = request.getParameter("action");
        if (action == null) {
            ...
            if (action.compareToIgnoreCase("newSynonym") == 0) {
                ...
                modelAndView = new ModelAndView(new RedirectView("alleleManagementDetail.emma?id=" + gene.getId_gene() + "&action=editGene"), "gene", gene);
                ...
                modelAndView = new ModelAndView(new RedirectView("alleleManagementList.emma"));
            }
        }
